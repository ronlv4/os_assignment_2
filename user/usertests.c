...
void klttest()
{
  ...
  printf("done malloc for stack_a\n");
  uint64 stack_b = (uint64)malloc(MAX_STACK_SIZE);
  printf("done malloc for stack_b\n");
  ...
  printf("join on kt_b returned %d\n", joined);
  if(joined != 0){
    printf("kthread_join failed\n");
    exit(1);
  }
